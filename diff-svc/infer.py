...
def run_clip(svc_model, key, acc, use_pe, use_crepe, thre, use_gt_mel, add_noise_step, project_name='', f_name=None,
    ...
    for (slice_tag, data) in audio_data:
        ...
        count += 1
    if out_path is None:
        out_path = f'./results/speech.{kwargs["format"]}'
...
if __name__ == '__main__':
    ...
    project_name = "test"
    model_path = f'./checkpoints/{project_name}/model_ckpt_steps_70000.ckpt' # change ckpt file name to your best ckpt file name
    config_path = f'./checkpoints/{project_name}/config.yaml'

    # Support multiple wav/ogg files, put them in the raw folder, with extension
    file_names = ["text.wav"]
    trans = [-1] # Pitch adjustment, 
